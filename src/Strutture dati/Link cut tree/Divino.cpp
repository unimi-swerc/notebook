...
/// Verification:
/// https://dmoj.ca/submission/5761839
//link cut tree degli dei ($\mathcal{O}(\log{N})$)
#define fori(a,b) for(int i=(a);i<(b);i++)
//0.714s con $n,q \leq 10^5$, 2.234s con $n,q \leq 2\cdot 10^5$
const int N=100010;
...
lazy operator+=(lazy& a,lazy& b){
  a.a=a.a*b.a; a.b=a.b*b.a+b.b; return a;
}
struct Info{
  int sz,sum,mi,ma; //mettere ll se necessario
  Info(int v){ sz=1; sum=mi=ma=v; }
  Info(){
...
struct node{
  int id,val; sp p,c[5];
  bool flip; Info info[2]; lazy lz[2];
  node(int i, int v){
    id=i; val=v;
    lz[0]=lz[1]={1,0}; info[0]=Info(val);
    p=nullptr; fori(0,5) c[i]=nullptr; flip=0;
  }
  void upd(lazy l){ lz[0]+=l; info[0]+=l; val+=l; }
  void upd2(lazy l){ lz[1]+=l; info[1]+=l; }
  void push(){
    if(flip){
      swap(c[0],c[1]);
  ...
  void rot(){
    ...
    connect(this,x,d^1); x->pull(); pull();
  }
};
void splay(sp x){
  if(!x) return;
...
void access(sp x){
  for(sp v=x,last=nullptr;v;v=v->p){
    ...
    if(last){
      ...
      if(a) a->p=nullptr; if(b) b->p=nullptr;
...
void cut(sp x){
  access(x); x->c[0]->p=nullptr; x->c[0]=nullptr;
}
sp lct[N];int n,q,a,b,c,t,rt;pair<int,int> e[N];Info ans;
int main(){
  ios::sync_with_stdio(false);cin.tie(0);cin>>n>>q;
  ...
  fori(0,q){
    ...
    if(t==1){ cin>>a; rt=a; }//change root
      ...
      cin>>a>>b>>c; evert(lct[a]); access(lct[b]);
      lct[b]->upd({t==6,c});
    }
