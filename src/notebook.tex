...
\newcommand{\maketeampage}{
	...
	\begin{center}
		\strut % Magic
		\vspace{3cm}\\
		\includegraphics[width=5cm]{unimi.pdf}\\
		{\fontsize{25}{30}\selectfont Università degli Studi di Milano\\}
		{\fontsize{60}{60}\selectfont The LaSt(atale) Dance\\}
		\vspace{0.5cm}
		{\huge Nicolas Albani, Fabrizio Brioni, Michael Chelli\\}
		\vfill
		{\fontsize{21}{25}\selectfont Southwestern Europe Regional Contest 2024-2025}\\
		\vspace{0.5cm}
		{\huge 1 Dicembre 2024}\\
		\vspace{1cm}
	\end{center}
	\clearpage
	\pagestyle{fancy}
...
\lhead{Università degli Studi di Milano - The LaSt(atale) Dance}
\chead{
...
% Title/Author
\author{The LaSt(atale) Dance 2024-2025}
\title{Team Reference Document}
\date{\vspace{-1.7em} 1 Dicembre 2024} %\today{}}
...
\node[text width=9.3cm,align=center](Text){{\fontsize{7pt}{7pt} \selectfont \tableofcontents}};
...
for _, section in ls(current_dir) do
    if not section:find("%.") then
        ...
        tex.print("\\sectionOrnament{" .. section:gsub("^%l", string.upper) .. "}{" .. sectionOrnaments[sectionIndex] .. "}{" .. sectionOrnamentSizes[sectionIndex] .. "}{" .. sectionOrnamentOffsetx[sectionIndex] .. "}{" .. sectionOrnamentOffsety[sectionIndex] .. "}")
...
\end{luacode*}
\hfill \fbox{\phantom{\rule{.7ex}{.7ex}}}
