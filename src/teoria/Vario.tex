...
Let $X$ be a discrete random variable with probability $p_X(x)$ of assuming the value $x$. Then $\mu=\mathbb{E}(X)=\sum_xxp_X(x)$ and $\sigma^2=V(X)=\mathbb{E}(X^2)-(\mathbb{E}(X))^2=\sum_x(x-\mathbb{E}(X))^2p_X(x)$. If $X$ is instead continuous it will have a probability density function $f_X(x)$ and the sums above will instead be integrals with $p_X(x)$ replaced by $f_X(x)$.
Moreover it holds $\mathbb{E}(aX+bY) = a\mathbb{E}(X)+b\mathbb{E}(Y)$ for any (even dependent) $X,Y$.
For independent $X$ and $Y$: $V(aX+bY) = a^2V(X)+b^2V(Y)$.
For dependent $X$ and $Y$: $V(X+Y) = V(X) + V(Y) + 2\mathit{Cov}(X,Y)$ where $\mathit{Cov}(X,Y) = \mathbb{E}[(X-\mathbb{E}[X])(Y-\mathbb{E}[Y])] = \mathbb{E}[XY] - \mathbb{E}[X]\mathbb{E}[Y]$.
Moreover it holds $\mathit{Cov}(aX+b,Y) = a\mathit{Cov}(X,Y)$ and $\mathit{Cov}(X+Y,Z) = \mathit{Cov}(X,Z)+\mathit{Cov}(Y,Z)$.
