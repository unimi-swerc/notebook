...
Moreover it holds: 
$$\mathbb{E}(aX+bY) = a\mathbb{E}(X)+b\mathbb{E}(Y) \text{\ for any (even dependent) $X,Y$}$$
$$V(aX+bY) = a^2V(X)+b^2V(Y) \text{\ for independent $X$ and $Y$}$$
$$V(X+Y) = V(X) + V(Y) + 2\mathit{Cov}(X,Y) \text{\ for dependent $X$ and $Y$}$$ $$\mathit{Cov}(X,Y) = \mathbb{E}[(X-\mathbb{E}[X])(Y-\mathbb{E}[Y])] = \mathbb{E}[XY] - \mathbb{E}[X]\mathbb{E}[Y]$$
